...
2. **Dynamic Permission Matrices:** Define granular permission sets using the standardised `resource:action` format (e.g., `prompts:view`, `prompts:review`, `prompts:approve`, `prompts:delete`, `users:view`, `users:suspend`, `admins:create`, `logs:view`) for custom admin roles (`/admin/roles`).
