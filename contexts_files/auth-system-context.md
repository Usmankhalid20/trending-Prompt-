...
- **Pending** – Creator registration is waiting for Admin approval.
- **Approved** – Creator can login and access Creator Portal.
- **Rejected** – Creator cannot use Creator Portal functionality.
- **Suspended** – Creator access is disabled by an authorized Admin/Super Admin.
...
Standardised permission naming: `resource:action` (e.g., `user:view`, `creator:approve`, `prompt:delete`).
...
Admin login should use: `/admin/login`
...
- **Guest:** Can access `/`, `/explore`, `/login`, `/register`
- **User:** Can access `/dashboard`
- **Approved Creator:** Can access `/creator`
- **Pending Creator:** Cannot access Creator Portal functionality.
- **Admin:** Can access `/admin` according to assigned permissions.
- **Super Admin:** Can access all administrative functionality.
...
For example, hiding "Approve Prompt" from a Creator is NOT a security mechanism.

The backend must also check:
1. Is authenticated?
2. Is correct role?
3. Has required permission?
4. Is resource accessible?
5. Allow request
...
- **Approved Creator:** `/login` → `/creator`
- **Pending Creator:** `/login` → Pending Approval State
...
## 40. Additional Security Features

### IP-Based Rate Limiting
- **Implementation:** `lib/rate-limit.ts` (sliding‑window strategy)
- **Protected endpoints:** `/api/auth/login`, `/api/auth/register`
- **Configuration:** 5 attempts per 60 seconds per IP
- **Response:** 429 Too Many Requests when limit exceeded
- **Integrated:** Yes, fully active in production

### CSRF Protection
- **Status:** Recommended for all state‑changing operations (POST, PUT, DELETE)
- **Implementation:** Use csrf package or double‑submit cookies
- **Plan:** To be added in a future security patch

### Environment Validation
All environment variables (especially `JWT_SECRET`, `MONGODB_URI`) must be validated on startup.
- **Implementation:** Use Zod schema in `lib/env.ts` and throw on missing/invalid values.

---

## 41. Final Authentication Architecture
...
Maintain one authentication system, not completely separate authentication implementations for User, Creator, Admin, and Super Admin.
